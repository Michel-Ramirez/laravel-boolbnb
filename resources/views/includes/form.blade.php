...
<div class="row">
  ...
  <div class="col-12">
    <div class="mb-3">
      ...
      <input type="text" class="form-control rounded-0 rounded-top @error('home_address') is-invalid @enderror"
        id="address" name="home_address" value="{{ old('home_address', $house->address->home_address ?? null) }}"
  ...
  <div class="col">
    <div class="mb-3">
      ...
      <input type="number" class="form-control @error('total_bath') is-invalid @enderror" id="bath"
        name="total_bath" value="{{ old('total_bath', $house->total_bath) }}" min="0" max="255" required>
      @error('total_bath')
        <div class="invalid-feedback">{{ $message }}</div>
      @enderror
    </div>
  ...
  <div class="col">
    <div class="mb-3">
      ...
      <input type="number" class="form-control @error('total_rooms') is-invalid @enderror" id="rooms"
        name="total_rooms" value="{{ old('total_rooms', $house->total_rooms) }}" min="0" max="255" required>
      @error('total_rooms')
        <div class="invalid-feedback">{{ $message }}</div>
      @enderror
    </div>
  ...
  <div class="col">
    <div class="mb-3">
      ...
      <input type="number" class="form-control @error('total_beds') is-invalid @enderror" id="beds"
        name="total_beds" value="{{ old('total_beds', $house->total_beds) }}" min="0" max="255" required>
      @error('total_beds')
        <div class="invalid-feedback">{{ $message }}</div>
      @enderror
    </div>
  ...
  <div class="col">
    <div class="mb-3">
      ...
      <input type="number" class="form-control @error('mq') is-invalid @enderror" id="mq" name="mq"
        value="{{ old('mq', $house->mq) }}" min="0" max="32000">
      @error('mq')
        <div class="invalid-feedback">{{ $message }}</div>
      @enderror
    </div>
  ...
  <div class="col-12 mt-3">
    <div class="form-check form-switch">
      ...
      <input class="form-check-input @error('is_published') is-invalid @enderror" name="is_published" value="1"
        type="checkbox" role="switch" id="published" @if (old('is_published', $house->is_published)) checked @endif>
      @error('is_published')
        <div class="invalid-feedback">{{ $message }}</div>
      @enderror
    </div>
    ...
    @foreach ($services as $service)
      <div class="form-check">
        <input class="form-check-input" type="checkbox" value="{{ $service->id }}"
          id="service-{{ $service->id }}" name="service[]" @if (in_array($service->id, old('service', $servicesIdArray ?? []))) checked @endif>
